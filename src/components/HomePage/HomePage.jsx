...
export const HomePage = () => {
	...
	const onGenerateTrip = async () => {
		...
		try {
			...
			if (!result) throw new Error("No result returned from AI service");
			if (!result.response)
				throw new Error("AI service returned no response object");

			if (typeof result.response.text !== "function") {
				// If response.text isn't a function, try to use it directly
				...
				throw new Error("AI response has no text() method");
			}
			...
			try {
				...
				console.error("Failed to parse AI response as JSON:", err, responseText);
				throw new Error("AI returned invalid JSON. Check console for full response.");
			}

			saveAiTrip(responseJSON);
		} catch (error) {
			console.error("Error generating trip:", error, result);
			toast("Failed to generate trip: " + (error?.message || "unknown error"));
		} finally {
			setLoading(false);
		}
	};
	...
	return (
		<>
			<div className="w-full overflow-visible px-3 md:px-14 lg:px-14 xl:px-40 font-serif ">
